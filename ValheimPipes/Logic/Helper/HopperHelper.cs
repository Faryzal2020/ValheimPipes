...
namespace ValheimPipes.Logic.Helper {
    public static class HopperHelper {
        public static event Action<ITarget> OnTargetChanged;

        public static void NotifyChange(ITarget target) {
        ...
        public static List<T> FindTargets<T>(Vector3 pos, Vector3 size, Quaternion rotation, Func<T, HopperPriority> orderBy, ITarget exclude) where T : ITarget {
            ...
            for (int i = 0; i < count; i++) {
                GameObject go = tempColliders[i].gameObject;
                int id = go.GetInstanceID();
                if (checkedObjects.Contains(id)) continue;
                checkedObjects.Add(id);

                T[] possibleTargets = go.GetComponentsInParent<T>();
                foreach (T target in possibleTargets) {
                    if (target.InRange(pos) && target.IsValid() && target.NetworkHashCode() != exclude.NetworkHashCode()) {
                        targets.Add(target);
                    }
                }
            }
